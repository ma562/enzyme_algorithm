...
% from the csv. It computes possible km and vmax values based on two
% constraint equations using two equations and two unknowns. This will
% yield 90 distinct pairs of km and vmax. We then mix up all the possible
% pairs of km and vmax to get 8100 possible pairs of km and vmax. Each km
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% New code: takes several permutations of points
SSE = realmax;      %%max possible value such that the first sse will for sure be lower than this

km = zeros(1, 2*length(test_points(:, 1)));     %%preallocated vector of km length of 2 * 45 = 90
vmax = zeros(1, 2*length(test_points(:, 1)));   %%preallocated vector of vmax length of 2 * 45 = 90
...
for i = 1:length(test_points(:, 1))
    ...
    ind = ind + 1;
    %%derivative equation was found by taking the line equation tangent to
    %%the model equation at point (km, 0.5vmax) and algebraically solving 2
    %%equations and 2 unknowns
    km(ind) = (s1*v2 - s2*v1)/(v1-v2);          %%km value based on derivative of original equation
    vmax(ind) = v2 / ((s2 + km(ind)) / (4*km(ind)));    %%vmax value based on derivative of original equation
    ind = ind + 1;
end
